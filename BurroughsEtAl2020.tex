...
monkeys correlates with complex spike duration \cite{YangLispberger2014}. It has even been proposed that the refractory period which follows a complex spike has a role in plasticity at the granule cell - Purkinje cell synapse \cite{Houghton2014}.
...
which are broadly typical to Purkinje cells; the five-current model in
addition, simulated the extended refractory period which follows
complex spikes. Table~\ref{table1} gives a list of the model parameters, giving the source for parameters that have been fixed to an experimental value and noting which parameters where adjusted to fit the target complex spike.
...
increases in the rate of simple spike firing. To indicate the relationship between the complex spike and the input current these are plotted together in Fig.~\ref{fig:S1_fig} for the specific complex spike seen in Fig.~\ref{injection_amplitudes}Biv.
...
the three-current model does a poor job of modelling the interaction
...
chosen as a target because it typifies features of the complex spike
...
\end{enumerate}
These typical features are exemplified by the fitted model. The
comples spike recorded in \cite{DavieEtAl2008} was also used as a
target beause the complex spike waveform was recorded there in
...
use as a target for model fitting. The five-current model also captures the extended refractory period that follows a complex spike; since this feature is easily seen in extra-cellular recordings, it is supported by a larger range of \textsl{in vivo} data, for example 
...
dendritic EPS is lost during transmission to the soma and there is a
...
since one-compartment models are considerably more transparent, this
suggests a one-compartment model is useful at this level of
approximation.
