...
point for a model which also incorporate the interaction between
...
complex spikes are principally sodium spikes \cite{StuartHausser1994},
...
can be generated by a limited number of channel-dynamics. The
...
with experiments \cite{RamanBean1997}. After the initial spike of the
complex spike, channels become sequestered in the open-but-blocked
state so that the following spikelets are stunted relative to the
initial spike and blocked channels returning to the open state help
sustain the depolarization for subsequent spikelet generation.
